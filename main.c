...
static int usb_probe(struct usb_interface *usb_intf,
		      const struct usb_device_id *id)
{
...
static void usb_disconnect(struct usb_interface *usb_intf)
{
	struct usb_skel *dev;
...
//	int minor = usb_intf->minor;
	...
	/* give back our minor */
	printk(KERN_INFO "WNA: deregistering USB device... \n");	
	usb_deregister_dev(usb_intf, &skel_class);
	printk(KERN_INFO "WNA: USB device deregistered. \n");	

	printk(KERN_INFO "WNA: Skipped:  killing anchored urbs.\n");	
	printk(KERN_INFO "WNA: Skipped:  decreasing usage count.\n");
	printk(KERN_INFO "WNA: Now disconnected. \n");
